...
\begin{description}
    ...
    \item[Difficult Terrain:] These terrain features make movement more difficult. Bases move at half speed while moving through them. Ruins and forests are good examples of Difficult Terrain. \textit{(Inc)}

    \item[Dangerous Terrain (X+/AP Y):] These terrain features are hazardous to cross. A base that moves through Dangerous Terrain, or activates while within it, suffers a hit on a roll of X+ with AP~Y. This is treated as a shooting attack that ignores cover saves.
...
\begin{description}
    \item[Occulting:] These terrain features completely block line of sight. Rocks and hills are good examples.

    If bases can occupy an Occulting terrain feature, line of sight is blocked as soon as it passes through the terrain. A base must therefore be positioned at the edge of the terrain feature to have line of sight beyond it.
...
\begin{description}
    ...
    Class 1--3 bases crossing a barricade lose 5~cm of movement. \textit{(Inc)}
    ...
    \item[Dragon's Teeth:] Dragon's Teeth are designed to block vehicle movement. They have no effect on line of sight. \textit{(Inc)}

    \item[Forests:] A forest is an Occulting terrain feature that provides a 5+ cover save to every base occupying it whose class is equal to or lower than the forest's designated class.

    If a line of sight passes through a forest, it is blocked. If a base is more than twice the size of the forest, it has unrestricted line of sight. If a base is inside a forest, a valid line of sight must pass directly from the base without passing through any other part of the forest. \textit{(Inc)}
    ...
    \item[Marsh:] Marshes provide a 5+ cover save to Class 1 and 2 bases. They count as Difficult Terrain for all classes except Titans and Praetorians. They are also \textit{Dangerous Terrain (6+/None)} for Cavalry and Vehicles. \textit{(Inc)}

    \item[Rivers:] Rivers count as Difficult Terrain and \textit{Dangerous Terrain (5+/None)} for infantry. Apart from Titans and Praetorians, no other bases may cross them. \textit{(Inc)}
    ...
    \item[Bastion:] Bastions are large bunkers with a Titan-class weapon mounted on the roof. This weapon is included in their points cost and gains the \textit{Turret} and \textit{Autonomous Defence} abilities.

    A Bastion provides a 3+ cover save and a $+2$ AF bonus. It has a 3+ Save, 4 Wounds, and a transport capacity of 10 infantry bases.

    \item[Bunker:] Bunkers are either large bunkers or complexes consisting of several smaller defensive positions.

    A Bunker provides a 3+ cover save and a $+2$ AF bonus. It has a 3+ Save, 3 Wounds, and a transport capacity of 10 infantry bases.

    \item[Artillery Emplacement:] Artillery Emplacements are large bunkers designed to protect vehicles. They provide a 3+ cover save and a $+2$ AF bonus, and have a 3+ Save and 3 Wounds.

    Infantry, Walker, and Cavalry bases may enter them, but then receive only the 3+ cover save.

    An Artillery Emplacement has a capacity of either three Vehicle bases or six Infantry, Walker, or Cavalry bases. \textit{(Inc)}
...
\begin{table}[ht]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tblr}{
            ...
            \textbf{Forest} &
            \SetCell[c=2]{c,m} Normal \textit{(Inc)} & &
            \SetCell[c=3]{c,m} Difficult \textit{(Inc)} & & &
            Normal &
            \SetCell{c,m}{Occulting\\Class 4} &
            Indestructible &
            5+ &
            0
            \\
            ...
            \textbf{Marsh} &
            Difficult &
            \SetCell[c=2]{c,m}{Difficult,\\Dangerous (6+/None) \textit{(Inc)}} & &
            ...
            \SetCell{c,m}{Difficult,\\Dangerous (5+/None) \textit{(Inc)}} &
            ...
            \SetCell[c=2]{c,m} Normal & &
            \SetCell{c,m}{Occulting\\Class 1 \textit{(Inc)}} &
            \SetCell{c,m}{7+\\1 Wound per 10~cm \textit{(Inc)}} &
            5+ &
            $+1$
            \\
            ...
            \SetCell{c,m}{Occulting\\Classes 4--7} &
            ...
            \SetCell[c=4]{c,m}
                May enter; otherwise Impassable \textit{(Inc)} & & & &
            \SetCell[c=2]{c,m} Impassable \textit{(Inc)} & &
            \SetCell{c,m}{Occulting\\Class 3} &
            \SetCell{c,m}{3+\\3 Wounds} &
            4+ \textit{(Inc)} &
            $+2$ \textit{(Inc)}
            ...
            \SetCell{c,m}{\textbf{Dragon's}\\\textbf{Teeth}} &
            \SetCell[c=3]{c,m} Normal & & &
            Impassable &
            Difficult \textit{(Inc)} &
            Normal &
            No Effect &
            \SetCell{c,m}{5+\\2 Wounds \textit{(Inc)}} &
            -- &
            0
        \end{tblr}%
    }
